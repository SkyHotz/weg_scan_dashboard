...
def load_excel_data(file_path):
    ...
    try:
        ...
        numeric_cols = ['VIBRAÇÃO AXIAL(mm/s)', 'VIBRAÇÃO RADIAL-Y (mm/s)', 
                       'VIBRAÇÃO RADIAL-X (mm/s)', 'TEMPERATURA(°C)']
...
def export_to_excel(df):
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            # Planilha principal
            df_export = df[['DateTime', 'EQUIPAMENTO', 'VIBRAÇÃO AXIAL(mm/s)', 
                           'VIBRAÇÃO RADIAL-Y (mm/s)', 'VIBRAÇÃO RADIAL-X (mm/s)', 
                           'TEMPERATURA(°C)']].copy()
            df_export.columns = ['Data/Hora', 'Equipamento', 'Vibração Axial (mm/s)',
                                'Vibração Radial-Y (mm/s)', 'Vibração Radial-X (mm/s)', 'Temperatura (°C)']
            ...
            for equipment in df['EQUIPAMENTO'].unique():
                ...
                summary_data.append({
                    ...
                    'Registros': len(df_eq),
                    'Vibração Axial Média': df_eq['VIBRAÇÃO AXIAL(mm/s)'].mean(),
                    'Vibração Radial-Y Média': df_eq['VIBRAÇÃO RADIAL-Y (mm/s)'].mean(),
                    'Vibração Radial-X Média': df_eq['VIBRAÇÃO RADIAL-X (mm/s)'].mean(),
                    'Temperatura Média': df_eq['TEMPERATURA(°C)'].mean()
                })
...
with st.sidebar:
    ...
    if st.session_state.data is not None:
        ...
        # Filtro de variável
        variables = ['VIBRAÇÃO AXIAL(mm/s)', 'VIBRAÇÃO RADIAL-Y (mm/s)', 
                    'VIBRAÇÃO RADIAL-X (mm/s)', 'TEMPERATURA(°C)']
        ...
        with col_export2:
            if st.button("📋 CSV", use_container_width=True):
                csv_data = st.session_state.data[['DateTime', 'EQUIPAMENTO', 'VIBRAÇÃO AXIAL(mm/s)', 
                                                   'VIBRAÇÃO RADIAL-Y (mm/s)', 'VIBRAÇÃO RADIAL-X (mm/s)', 
                                                   'TEMPERATURA(°C)']].to_csv(index=False)
                st.download_button(
                    label="⬇️ Baixar",
                    data=csv_data,
...
if st.session_state.data is not None:
    ...
    if df_filtered.empty:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["📈 Gráficos", "📊 Estatísticas", "⚠️ Alertas", "📋 Dados"])
        ...
        with tab4:
            ...
            df_display = df_filtered[['DateTime', 'EQUIPAMENTO', 'VIBRAÇÃO AXIAL(mm/s)', 
                                     'VIBRAÇÃO RADIAL-Y (mm/s)', 'VIBRAÇÃO RADIAL-X (mm/s)', 
                                     'TEMPERATURA(°C)']].copy()
...
<div style='text-align: center; color: #666; font-size: 12px;'>
    <p>WEG SCAN Dashboard | Monitoramento de Equipamentos | Desenvolvido com Streamlit</p>
</div>
""", unsafe_allow_html=True)
